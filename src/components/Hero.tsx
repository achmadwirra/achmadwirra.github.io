"use client";

import { motion } from "framer-motion";
import { Download, ArrowRight } from "lucide-react";

const GithubIcon = () => (
  <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
  </svg>
);

const LinkedinIcon = () => (
  <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
  </svg>
);

const XIcon = () => (
  <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
  </svg>
);

export default function Hero() {
  return (
    <section className="relative min-h-screen flex items-center pt-16 overflow-hidden grid-pattern">
      {/* Background gradient */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#0a0f1a] via-[#0d1117] to-[#0a0f1a]" />
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#06b6d4]/5 rounded-full blur-3xl" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-[#06b6d4]/3 rounded-full blur-3xl" />

      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          {/* Left side */}
          <motion.div
            initial={{ opacity: 0, x: -50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.7 }}
            className="order-1"
          >
            <span className="text-[#06b6d4] text-sm font-semibold tracking-widest uppercase mb-4 block">
              I&apos;m Developer
            </span>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-[#f8fafc] mb-6 leading-tight">
              ACHMAD
              <br />
              <span className="text-[#06b6d4]">WIRA</span>
            </h1>
            <p className="text-[#94a3b8] text-lg mb-8 max-w-lg leading-relaxed">
              Full Stack Developer crafting modern web applications, AI-powered
              tools, and Web3 solutions. Turning complex ideas into elegant,
              performant digital experiences.
            </p>

            <div className="flex flex-wrap gap-4 mb-8">
              <a
                href="#contact"
                className="inline-flex items-center gap-2 bg-[#06b6d4] hover:bg-[#22d3ee] text-[#0a0f1a] px-6 py-3 rounded-full font-semibold transition-colors"
              >
                Hire Me <ArrowRight size={18} />
              </a>
              <a
                href="/cv.pdf"
                download
                className="inline-flex items-center gap-2 border border-[#1e293b] hover:border-[#06b6d4] text-[#f8fafc] px-6 py-3 rounded-full font-semibold transition-colors"
              >
                Download CV <Download size={18} />
              </a>
            </div>

            <div className="flex items-center gap-4">
              <a
                href="https://github.com/achmadwira"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 rounded-full border border-[#1e293b] flex items-center justify-center text-[#94a3b8] hover:text-[#06b6d4] hover:border-[#06b6d4] transition-colors"
              >
                <GithubIcon />
              </a>
              <a
                href="https://linkedin.com/in/achmadwira"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 rounded-full border border-[#1e293b] flex items-center justify-center text-[#94a3b8] hover:text-[#06b6d4] hover:border-[#06b6d4] transition-colors"
              >
                <LinkedinIcon />
              </a>
              <a
                href="https://x.com/fekzsya"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 rounded-full border border-[#1e293b] flex items-center justify-center text-[#94a3b8] hover:text-[#06b6d4] hover:border-[#06b6d4] transition-colors"
              >
                <XIcon />
              </a>
            </div>
          </motion.div>

          {/* Right side - Profile Photo */}
          <motion.div
            initial={{ opacity: 0, x: 50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.7, delay: 0.2 }}
            className="relative flex justify-center order-2"
          >
            <div className="relative">
              {/* Glow behind photo */}
              <div className="absolute inset-0 rounded-2xl bg-[#06b6d4]/20 blur-2xl scale-110" />

              {/* Profile photo frame */}
              <div className="relative w-64 h-64 sm:w-72 sm:h-72 md:w-80 md:h-80 rounded-2xl border-2 border-[#06b6d4]/40 overflow-hidden shadow-[0_0_40px_rgba(6,182,212,0.15)]">
                <img
                  src="https://avatars.githubusercontent.com/u/33767655?v=4"
                  alt="Achmad Wira"
                  className="w-full h-full object-cover"
                />
              </div>

              {/* Floating badges */}
              <motion.div
                animate={{ y: [0, -8, 0] }}
                transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                className="absolute -top-4 -right-4 bg-[#1a1f2e]/90 backdrop-blur-sm border border-[#06b6d4]/30 rounded-lg px-3 py-2 text-xs font-semibold text-[#06b6d4] shadow-[0_0_15px_rgba(6,182,212,0.1)]"
              >
                ⚡ Full Stack
              </motion.div>
              <motion.div
                animate={{ y: [0, 8, 0] }}
                transition={{
                  duration: 3,
                  repeat: Infinity,
                  ease: "easeInOut",
                  delay: 1,
                }}
                className="absolute -bottom-4 -left-4 bg-[#1a1f2e]/90 backdrop-blur-sm border border-[#22d3ee]/30 rounded-lg px-3 py-2 text-xs font-semibold text-[#22d3ee] shadow-[0_0_15px_rgba(34,211,238,0.1)]"
              >
                🔗 Web3
              </motion.div>
              <motion.div
                animate={{ y: [0, -6, 0] }}
                transition={{
                  duration: 3,
                  repeat: Infinity,
                  ease: "easeInOut",
                  delay: 2,
                }}
                className="absolute top-1/2 -translate-y-1/2 -right-12 sm:-right-14 bg-[#1a1f2e]/90 backdrop-blur-sm border border-[#00d4ff]/30 rounded-lg px-3 py-2 text-xs font-semibold text-[#00d4ff] shadow-[0_0_15px_rgba(0,212,255,0.1)]"
              >
                🤖 AI Developer
              </motion.div>
            </div>
          </motion.div>
        </div>
      </div>
    </section>
  );
}
